...
> = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const tournamentId =
      args.tournamentId ??
      (await ctx.runQuery(
        internal.functions.cronTeamsInternal.getActiveTournamentIdForTeamsCron,
        {},
      ));
    ...
    const snap = (await ctx.runQuery(
      internal.functions.cronTeamsInternal.getTournamentSnapshotForTeamsCron,
      { tournamentId },
    )) as TournamentSnap;
    ...
    const playoff = (await ctx.runQuery(
      internal.functions.cronTeamsInternal.computePlayoffContext,
      { tournamentId },
    )) as PlayoffContext;
    ...
    return await ctx.runMutation(
      internal.functions.cronTeamsInternal.applyTeamsUpdate,

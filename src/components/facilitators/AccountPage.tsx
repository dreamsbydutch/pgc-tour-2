...
export function AccountPage() {
  ...
  if (vm.isLoading) return <AccountPageSkeleton />;
  if (!vm.overview) return null;

  return (
    <div className="bg-white pb-24">
      <AccountOverview overview={vm.overview} vm={vm} />
    </div>
  );
}

function AccountOverview(props: ReadyProps) {
  ...
  return (
    <div>
      ...
      <Button
        ...
        disabled={
          !props.vm.canSubmitSettlement ||
          props.vm.submitting ||
          !props.vm.parsedAmounts.valid ||
          props.vm.parsedAmounts.remainingCents !== 0
        }
        onClick={props.vm.onSubmitSettlement}
      >
        {props.vm.submitting ? "Submitting…" : "Submit payout instructions"}
      </Button>
      {props.vm.submitError ? (
        <p role="alert" className="mt-3 text-sm text-red-700">
          {props.vm.submitError}
        </p>
      ) : null}
      {props.vm.submitSuccess ? (
        <p role="status" className="mt-3 text-sm text-golf-700">
          {props.vm.submitSuccess}
        </p>
      ) : null}
    </div>
  ...
  return (
    ...
    >
      ...
      <Button
        type="button"
        className="mt-4"
        onClick={props.vm.onSaveProfile}
        disabled={props.vm.saving}
      >
        {props.vm.saving ? "Saving…" : "Save changes"}
      </Button>
      {props.vm.saveError ? (
        <p role="alert" className="mt-3 text-sm text-red-700">
          {props.vm.saveError}
